...
from maivn.events._bridge.schema import EventSchemaError, ValidationMode, validate_event
...
def test_concurrent_producers_preserve_all_events_in_history() -> None:
    ...
    async def _run() -> tuple[int, int]:
        ...
        _ = await asyncio.gather(*[producer(p, 200) for p in range(10)])
...
def test_reconnect_mid_stream_replays_missing_events_in_order() -> None:
    ...
    async def _run() -> tuple[list[str], list[int]]:
        ...
        history = bridge.get_history()
        cursor = cast(str, history[4]["id"])  # client saw events 0..4
        ...
        try:
            async for sse_event in gen:
                if "id" not in sse_event:
                    continue
                payload = cast(dict[str, object], json.loads(cast(str, sse_event["data"])))
                seen_ids.append(cast(str, sse_event["id"]))
                payload_data = cast(dict[str, int], payload["data"])
                seen_indices.append(payload_data["i"])
                if payload_data["i"] == 9:
                    break
        finally:
            bridge.close()
...
def test_sse_generator_releases_cleanly_on_aclose_mid_stream() -> None:
    async def _run() -> bool:
        ...
        _ = await asyncio.wait_for(anext(gen), timeout=1.0)
        _ = await asyncio.wait_for(anext(gen), timeout=1.0)
...
def test_bridge_survives_cancelled_consumer_and_serves_a_new_one() -> None:
    ...
    async def _run() -> int:
        ...
        _ = await asyncio.wait_for(anext(first), timeout=1.0)
        await first.aclose()
        ...
        try:
            async for frame in gen:
                if "id" in frame:
                    events.append(cast(str, frame["event"]))
        finally:
            await gen.aclose()
        return len(events)
...
def test_bridge_constructor_rejects_invalid_validation_mode() -> None:
    # Intentional bad-input test: "loose" is not a ValidationMode literal.
    with pytest.raises(ValueError, match="schema_validation"):
        _ = EventBridge("s", schema_validation=cast(ValidationMode, cast(object, "loose")))
