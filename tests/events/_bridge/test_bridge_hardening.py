...
from maivn.events import BackpressurePolicy, EventBridge
...
def test_backpressure_drop_newest_drops_overflow() -> None:
    async def _run() -> tuple[int, list[int]]:
        ...
        while not bridge.stream_queue_empty():
            drained.append(cast(int, bridge.stream_queue_get_nowait().data["i"]))
...
def test_reopen_resets_identity_aliases_and_eviction_counter() -> None:
    from maivn.events._bridge.runtime.identity import BridgeIdentityState
    ...
    async def _run() -> tuple[dict[str, str], int]:
        ...
        evictions_before = bridge.stream_history_evictions
        identity_state = cast(
            BridgeIdentityState,
            getattr(bridge, "_identity_state"),  # noqa: B009
        )
        aliases_before = dict(identity_state.tool_id_aliases)
...
def test_reopen_clears_identity_state_so_new_turn_uses_fresh_ids() -> None:
    async def _run() -> tuple[str, str]:
        ...
        last_data = cast(dict[str, object], history[-1]["data"])
        return cast(str, last_data["tool_id"]), "turn2-id"
...
def test_safe_event_payload_preserves_id_and_type_on_failure() -> None:
    ...
    assert cast(dict[str, object], parsed["data"])["serialization_error"] is True
...
def test_serialization_handles_datetime_uuid_decimal_set_and_bytes() -> None:
    ...
    assert cast(str, parsed_data["when"]).startswith("2026-04-27")
    assert parsed_data["uid"] == "00000000-0000-0000-0000-000000000001"
    assert parsed_data["amount"] == "3.14"
    assert parsed_data["tags"] == sorted(["a", "b"], key=repr)
    assert parsed_data["blob"] == "hello"
...
def test_lost_cursor_after_history_overflow_logs_warning(
    ...
    async def _run() -> str:
        ...
        for i in range(10):
            await bridge.emit("evt", {"i": i})
            ids.append(cast(str, bridge.get_history()[-1]["id"]))
...
def test_history_evictions_count_increments_when_buffer_full() -> None:
    async def _run() -> int:
        ...
        for i in range(7):
            await bridge.emit("evt", {"i": i})
        return bridge.stream_history_evictions
...
def test_bridge_drops_nested_payload_status_duplicate_when_enabled() -> None:
    """Regression (deferred row 102): the dedup drop-check must run against the
    NORMALIZED payload, so a duplicate whose canonical status text is nested
    differently in the raw payload is dropped consistently.
    """

    async def _run() -> int:
        bridge = EventBridge("dedup-status-nested", dedupe_status_messages=True)
        # First emit carries the canonical text at the top level.
        await bridge.emit("status_message", {"assistant_id": "orch", "message": "Working"})
        # Second emit nests the same canonical text under ``status`` (and the
        # assistant id under ``assistant``); pre-normalization fingerprinting
        # missed it and let the duplicate through.
        await bridge.emit(
            "status_message",
            {"assistant": {"id": "orch"}, "status": {"message": "Working"}},
        )
        return len([evt for evt in bridge.get_history() if evt["type"] == "status_message"])
...
def test_bridge_keeps_distinct_nested_status_messages_when_enabled() -> None:
    """Distinct canonical text must still pass through even when nested."""

    async def _run() -> list[str]:
        bridge = EventBridge("dedup-status-distinct", dedupe_status_messages=True)
        await bridge.emit("status_message", {"assistant_id": "orch", "message": "Working"})
        await bridge.emit(
            "status_message",
            {"assistant": {"id": "orch"}, "status": {"message": "Almost done"}},
        )
        return [
            cast(str, cast(dict[str, object], evt["data"])["message"])
            for evt in bridge.get_history()
            if evt["type"] == "status_message"
        ]

    assert asyncio.run(_run()) == ["Working", "Almost done"]
