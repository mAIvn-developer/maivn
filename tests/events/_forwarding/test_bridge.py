...
# The SDK's reporter contract is intentionally duck-typed (``reporter: object``
# on ``forward_normalized_stream``); the fake below records every call the
# forwarders make so we can assert on ordering and payload semantics.
...
class _Reporter:
    def __init__(self) -> None:
        ...
        self.response_chunks: list[dict[str, object]] = []
        self.status_messages: list[dict[str, object]] = []
        self.tool_starts: list[dict[str, object]] = []
        self.system_progress: list[dict[str, object]] = []
        self.tool_completes: list[dict[str, object]] = []
        self.tool_errors: list[dict[str, object]] = []
        self.model_completes: list[dict[str, object]] = []
        self.assignments: list[dict[str, object]] = []
        self.enrichments: list[dict[str, object]] = []
        self.events: list[tuple[str, str]] = []
        self.summaries: list[dict[str, object] | None] = []
    ...
    def report_enrichment(self, **kwargs: object) -> None:
        self.enrichments.append(dict(kwargs))

    def print_event(
        self, event_type: str, message: str, details: dict[str, object] | None = None
    ) -> None:
        _ = details
        self.events.append((event_type, message))

    def print_summary(self, token_usage: dict[str, object] | None = None) -> None:
...
def _event(payload: JsonObject) -> AppEvent:
    return AppEvent.model_validate(payload)


def _data(item: dict[str, object]) -> dict[str, object]:
    """Narrow ``item['data']`` to a concrete dict for typed indexing."""
    return cast("dict[str, object]", item["data"])


def _nested(value: dict[str, object], key: str) -> dict[str, object]:
    """Narrow ``value[key]`` to a dict for further typed indexing."""
...
def test_forward_normalized_stream_replays_reporter_semantics() -> None:
    ...
    _ = asyncio.run(forward_normalized_stream(events, reporter=reporter, state=state))
...
def test_forward_normalized_stream_replays_bridge_semantics() -> None:
    ...
    _ = asyncio.run(forward_normalized_stream(events, bridge=bridge))
    ...
    assert _nested(_data(history[0]), "session")["id"] == "sess-2"
    assert _data(history[4])["event_name"] == "system_tool_start"
    assert _nested(_data(history[5]), "chunk")["progress"] == 0.0
    assert _data(history[6])["event_name"] == "system_tool_complete"
    assert _nested(_data(history[7]), "tool")["type"] == "model"
    assert _nested(_data(history[7]), "tool")["status"] == "completed"
    assert _nested(_data(history[8]), "interrupt")["id"] == "int-1"
    assert _nested(_data(history[-1]), "output")["token_usage"] == {"total_tokens": 7}
