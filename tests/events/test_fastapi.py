...
def reset_sse_starlette_app_status() -> Iterator[None]:
    ...
    # sse-starlette declares should_exit_event as Event | None; resetting to
    # None lets the next request reinitialize against pytest's fresh loop.
...
def fresh_registry() -> Iterator[BridgeRegistry]:
    ...
    yield registry
    # Drain anything still alive. ``_bridges`` is intentional internal access
    # for the test fixture teardown — there's no public iteration API today.
    # ``cast(..., getattr(...))`` keeps the private read out of the
    # ``reportPrivateUsage`` path while preserving a concrete type at the use
    # site so ``reportAny`` stays quiet too.
    bridges_map = cast(dict[str, EventBridge], getattr(registry, "_bridges"))  # noqa: B009
    for session_id in list(bridges_map):
        registry.remove(session_id)


def _route_path(route: object) -> str | None:
    """Return ``route.path`` if it's a string, else ``None``.

    Starlette's ``BaseRoute`` doesn't declare ``path`` at the base class
    level; only ``APIRoute``/``Route`` subclasses carry it. The duck-typed
    getattr keeps the test contract narrow without leaking ``Any``.
    """
    raw = getattr(route, "path", None)
    return raw if isinstance(raw, str) else None


def _make_app(*, registry: BridgeRegistry) -> FastAPI:
    app = FastAPI()
    _ = mount_events(app, registry=registry)
    return app
...
def test_mount_events_registers_route_at_default_path(
    ...
    paths = {_route_path(route) for route in app.routes}
...
def test_create_event_router_rejects_path_without_session_id_placeholder() -> None:
    with pytest.raises(ValueError, match=r"\{session_id\}"):
        _ = create_event_router(path="/events")
...
def test_get_event_bridge_with_create_false_raises_for_missing(
    ...
    with pytest.raises(KeyError):
        _ = get_event_bridge("missing", create=False, registry=fresh_registry)
...
def test_auth_dependency_blocks_unauthorized_requests(
    ...
    app = FastAPI()
    _ = mount_events(app, auth=reject_all, registry=fresh_registry)
...
def test_auth_dependency_can_inspect_request(
    ...
    app = FastAPI()
    _ = mount_events(app, auth=require_token, registry=fresh_registry)
...
def test_route_signature_matches_sse_contract(
    ...
    """
    app = _make_app(registry=fresh_registry)
    [route] = [r for r in app.routes if _route_path(r) == "/maivn/events/{session_id}"]
    # FastAPI's APIRoute carries .methods + .dependant on the route subclass;
    # we duck-type access through cast(object, ...) so the strict checker
    # doesn't flag the BaseRoute base class missing those attrs.
    methods_raw = cast(object, getattr(route, "methods", None))
    methods = cast(set[str], methods_raw)
    assert "GET" in methods
    # The endpoint signature carries session_id (path) + last_event_id (query).
    dependant = cast(object, getattr(route, "dependant", None))
    path_params = cast(list[object], getattr(dependant, "path_params", []))
    query_params = cast(list[object], getattr(dependant, "query_params", []))
    param_names: set[str] = {cast(str, getattr(p, "name")) for p in path_params}  # noqa: B009
    param_names |= {cast(str, getattr(p, "name")) for p in query_params}  # noqa: B009
...
def test_helpful_error_when_fastapi_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    real_require = cast(object, getattr(fastapi_module, "_require_fastapi"))  # noqa: B009
    ...
    try:
        with pytest.raises(ModuleNotFoundError, match=r"maivn\[fastapi\]"):
            _ = fastapi_module.create_event_router()
    finally:
