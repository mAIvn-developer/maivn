...
def test_coerce_bool_value() -> None:
    # Canonical truthy tokens (case- and whitespace-insensitive).
    assert coerce_bool_value("true") is True
    assert coerce_bool_value("YES") is True
    assert coerce_bool_value("  on ") is True
    assert coerce_bool_value("1") is True

    # Canonical falsy tokens.
    assert coerce_bool_value("false") is False
    assert coerce_bool_value("no") is False
    assert coerce_bool_value("off") is False
    assert coerce_bool_value("0") is False
    assert coerce_bool_value("") is False

    # Unrecognized strings fall back to the explicit default.
    assert coerce_bool_value("maybe") is False
    assert coerce_bool_value("maybe", default=True) is True

    # Non-strings use standard truthiness.
    assert coerce_bool_value(1) is True
    assert coerce_bool_value(0) is False
    assert coerce_bool_value(True) is True
    assert coerce_bool_value(False) is False
