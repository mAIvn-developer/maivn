...
def test_depends_on_interrupt_detects_literal_choices() -> None:
    ...
    deps = cast(list[InterruptDependency], getattr(tool, "_dependencies", []))
    assert deps
    dep = deps[0]
...
def test_depends_on_interrupt_detects_boolean() -> None:
    ...
    dep = cast(list[InterruptDependency], getattr(tool, "_dependencies", []))[0]
...
def test_depends_on_tool_uses_callable_uuid() -> None:
    ...
    dep = cast(list[ToolDependency], getattr(tool, "_dependencies", []))[0]
...
def test_depends_on_agent_uses_agent_id_attribute() -> None:
    ...
    dep = cast(list[AgentDependency], getattr(tool, "_dependencies", []))[0]
...
def test_execution_control_decorators_attach_metadata_only_controls() -> None:
    ...
    controls = cast(
        list[AwaitForDependency | ReevaluateDependency],
        getattr(tool, "__maivn_execution_controls__", []),
    )
    assert len(controls) == 2
    assert cast(list[object], getattr(tool, "_dependencies", [])) == []
...
def test_compose_artifact_policy_attaches_arg_policy_metadata() -> None:
    ...
    policies = cast(list[ArgPolicy], getattr(tool, "__maivn_arg_policies__", []))
    assert policies == [
        {
            "arg_name": "query",
...
def test_dependency_decorators_validate_arg_name() -> None:
    ...
    with pytest.raises(ValueError):
        _ = depends_on_private_data("key", "missing")(tool)

    with pytest.raises(ValueError):
        _ = compose_artifact_policy("missing")(other_tool)
