...
class _QueueHttpClient:
    _responses: deque[dict[str, object] | list[dict[str, object]]]
...
def _resource_detail_payload() -> dict[str, object]:
...
def test_client_lists_memory_resources_with_filters_and_returns_models(
    ...
    parsed = urlparse(cast(str, http_client.calls[0]["url"]))
...
def test_client_manages_organization_memory_policy_and_purge(
    ...
    patch_json = cast(Mapping[str, object], http_client.calls[1]["json"])
    assert patch_json["persistence_ceiling"] == "vector_plus_graph"
...
def test_client_creates_memory_resource_and_returns_detail(
    ...
    assert cast(str, http_client.calls[0]["url"]).endswith("/projects/project-1/memory/resources")
...
def test_client_previews_redaction(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert http_client.calls[0]["method"] == "POST"
    assert cast(str, http_client.calls[0]["url"]).endswith("/preview-redaction")
    preview_json = cast(Mapping[str, object], http_client.calls[0]["json"])
    assert preview_json["private_data"] == {"existing": "value"}
