...
from maivn_shared import ToolDependency
...
def _tool_id_attr(obj: object) -> str:
    """Helper to read the dynamic ``tool_id`` attribute attached to callables."""
...
def test_agent_constructor_registers_callable_and_model_tools() -> None:
    def load_profile(customer_id: str) -> dict[str, object]:
    ...
    def build_recommendation(profile: dict[str, object]) -> dict[str, object]:
    ...
    assert _tool_id_attr(load_profile) == tools_by_name["load_profile"].tool_id

    recommendation_tool = tools_by_name["build_recommendation"]
    assert len(recommendation_tool.dependencies) == 1
    dep = recommendation_tool.dependencies[0]
    assert isinstance(dep, ToolDependency)
    assert dep.tool_id == tools_by_name["load_profile"].tool_id
...
def test_agent_add_tool_registers_callable_with_options() -> None:
    ...
    assert _tool_id_attr(classify_ticket) == tool.tool_id
...
def test_agent_constructor_accepts_prebuilt_tool_objects() -> None:
    ...
    assert _tool_id_attr(lookup_status) == prebuilt_tool.tool_id
