...
from maivn._internal.api.scheduling.models import RunRecord
...
async def test_runs_fire_and_callbacks_invoked() -> None:
    ...
    fire_records: list[RunRecord] = []
    success_records: list[RunRecord] = []
    error_records: list[RunRecord] = []

    job = builder.invoke("hello")
    _ = job.on_fire(lambda r: fire_records.append(r))
    _ = job.on_success(lambda r: success_records.append(r))
    _ = job.on_error(lambda r: error_records.append(r))
