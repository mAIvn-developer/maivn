...
class _Reporter:
    def __init__(self) -> None:
        ...
        self.response_chunk_calls: list[dict[str, object]] = []
    ...
    def report_tool_start(self, tool_name: str, event_id: str, **kwargs: object) -> None:
        self.start_calls.append({"tool_name": tool_name, "event_id": event_id, **kwargs})

    def report_system_tool_progress(self, **kwargs: object) -> None:
    ...
    def report_tool_complete(self, event_id: str, **kwargs: object) -> None:
        self.complete_calls.append({"event_id": event_id, **kwargs})

    def report_tool_error(self, tool_name: str, error: str, **kwargs: object) -> None:
...
def _as_reporter(reporter: object) -> BaseReporter | None:
    """Cast a duck-typed fake reporter to the BaseReporter slot.

    OrchestratorReporterHooks accepts a supplier returning BaseReporter | None
    but the runtime uses duck typing across the hook surface. Tests stub the
    surface with bare classes; the cast bridges the duck type to the
    declared BaseReporter slot.
    """
...
def test_reporter_hooks_model_complete_and_action_update() -> None:
    ...
    assert "private_data_injected" in result
...
def test_reporter_hooks_streaming_content_emits_response_deltas() -> None:
    reporter = _Reporter()
    hooks = OrchestratorReporterHooks(reporter_supplier=lambda: _as_reporter(reporter))
...
def test_reporter_hooks_streaming_content_handles_diverged_snapshots() -> None:
    ...
    assert reporter.response_chunk_calls == [
        ...
        {
            # Stream diverged (snapshot 2 doesn't share an extension prefix
            # with snapshot 1 — they only share "INVESTMENT MEMO: TechNova",
            # then snapshot 1 has ``\\`` and snapshot 2 has ``\n``). The
            # orchestrator hook tags this chunk as a fresh replacement AND
            # emits the FULL new cumulative text as the chunk payload, so
            # downstream reporters can overwrite the bubble with complete
            # content rather than just the divergent suffix.
            "text": "INVESTMENT MEMO: TechNova\n\n## EXECUTIVE SUMMARY",
            "assistant_id": "agent-1",
            "full_text": "INVESTMENT MEMO: TechNova\n\n## EXECUTIVE SUMMARY",
...
def test_reporter_hooks_nested_streaming_is_suppressed_by_default() -> None:
    reporter = _Reporter()
    hooks = OrchestratorReporterHooks(reporter_supplier=lambda: _as_reporter(reporter))
    hooks.configure_execution_context(is_nested=True, allow_nested_response_stream=False)
...
def test_reporter_hooks_nested_streaming_can_be_enabled() -> None:
    reporter = _Reporter()
    hooks = OrchestratorReporterHooks(reporter_supplier=lambda: _as_reporter(reporter))
    hooks.configure_execution_context(is_nested=True, allow_nested_response_stream=True)
...
def test_reporter_hooks_system_tool_events() -> None:
    ...
    start_event_id = reporter.start_calls[0]["event_id"]
    assert isinstance(start_event_id, str)
    assert start_event_id.startswith("system-tool:sys")
