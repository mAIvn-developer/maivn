...
def test_normalize_tools_handles_plain_objects() -> None:
    ...
    # Intentional duck-typed BaseTool stub: normalize_tools_for_structured_output
    # uses _optional_attr/getattr; the test verifies plain dataclasses still
    # work as the protocol-light tool surface the helper accepts.
    normalized = normalize_tools_for_structured_output(
        cast(list[BaseTool], cast(object, [plain])),
        cast(BaseTool, cast(object, structured)),
    )
...
def test_normalize_tools_reuses_existing_tool_with_same_model() -> None:
    ...
    # Duck-typed BaseTool stub (see test_normalize_tools_handles_plain_objects).
    normalized = normalize_tools_for_structured_output(
        cast(list[BaseTool], cast(object, [existing])),
        cast(BaseTool, cast(object, structured)),
    )
