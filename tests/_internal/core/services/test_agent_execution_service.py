...
class _Agent:
    ...
    def get_swarm(self) -> _Swarm | None:
        return self._swarm


class _Registry:
    _agent: AgentExecutionTarget
    agents: list[AgentExecutionTarget]

    def __init__(self, agent: AgentExecutionTarget) -> None:
        self._agent = agent
        self.agents = [agent]
...
def _as_registry(registry: _Registry) -> AgentRegistry:
    """Cast through ``object`` to bridge the duck-typed test registry."""
    return cast(AgentRegistry, cast(object, registry))
...
def test_agent_execution_service_resolves_and_executes() -> None:
    ...
    service = AgentExecutionService(agent_registry=_as_registry(registry))
...
def test_agent_execution_service_raises_without_registry() -> None:
    ...
    with pytest.raises(ValueError, match="agent"):
        _ = service.execute_agent_dependency(dependency, [])
