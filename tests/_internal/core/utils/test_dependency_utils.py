...
def test_normalize_dependencies_serializes_interrupt_dependency_without_handler() -> None:
    ...
    payload = cast(object, json.loads(normalized[0]))
    assert payload == {
...
def test_normalize_dependencies_keeps_present_falsy_identifier_values() -> None:
    normalized = normalize_dependencies([_EmptyToolIdDependency(), _ZeroToolIdDependency()])
